...
/*Function is enabling reading data from proc file*/ 
static ssize_t read_proc(struct file *filp, char __user *buffer, size_t length,loff_t * offset)
{ 
	//disabling multiple readings from file at the same time
    if(guard){
        guard = 0;
    }
    else{
...
/*Function expects input in next format: four numbers separated with comma. Atfer checking validity, function is writing static data into /proc file system */ 
static ssize_t write_proc(struct file *filp, const char *buff, size_t len, loff_t * off)
{
    int i,j;
...
static int __init stat_driver_init(void)
{
        ...
        /*Creating Proc entry*/
        proc_create("conway_stat_procfs",0666,NULL,&proc_fops);
...
{
        remove_proc_entry("conway_stat_procfs",NULL);
