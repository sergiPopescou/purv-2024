...
{
        for(int i= INT_SIZE_STATES - 1; i>=0; i--)
        {
                if(cell_states & (1 << i))
                {
        ...
        int8_t new_state = 0;
         ...
         switch(cmd) {
                ...
                /* solving problem about trying to read uninitialized state of cells */
                        ...
                        if(new_state)
                        {
                                SET_BIT(cell_state, SIZE);
                        }
                        else
                        {
                                CLEAR_BIT(cell_state, 5);
                        }
